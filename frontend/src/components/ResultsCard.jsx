...
function ResultsCard({text,voteNum,percentage}) {
  return (
    <div className="bg-[#4F1F73] h-[220px] w-full rounded-xl flex flex-col justify-center gap-5 px-6 py-6">
      <p className="text-xl sm:text-2xl font-medium sm:mb-5 leading-snug">{text}</p>
      <div className="flex flex-row justify-between items-center w-full mt-auto">
        <p className="text-lg sm:text-xl font-medium truncate">{voteNum} Votes</p>
        <p className="text-lg sm:text-xl font-medium text-gray-400 shrink-0">{percentage}</p>
      </div>

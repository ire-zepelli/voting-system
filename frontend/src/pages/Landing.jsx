    ...
    <div className="flex flex-col min-h-screen bg-[#34102A] text-white w-full overflow-x-hidden relative">
      ...
      <main className="relative z-10 mx-auto flex w-full max-w-7xl flex-1 px-6 sm:px-10 lg:px-16 pb-12 pt-8">
        <div className="w-full grid gap-10 lg:grid-cols-[1.25fr_0.75fr] lg:items-start">
          <section className="space-y-2 lg:pr-8 pt-4">
            {/* Logos */}
            ...
            <div className="space-y-[-1rem] text-white font-helvetica font-black relative z-10 break-words max-w-full">
              ...
              <div className="pointer-events-none absolute inset-0 sm:inset-x-0 z-[-1] flex items-center lg:items-start justify-center sm:justify-start lg:justify-center left-0 sm:left-[60%] lg:left-[55%]">
                {WORDS.map((word, idx) => (
                  <div
                    key={word}
                    className={`absolute inset-0 flex w-full justify-start font-helvetica text-[5.5rem] sm:text-[7.5rem] lg:text-[10.5rem] leading-none font-black tracking-tighter uppercase text-center sm:text-left transition-opacity duration-1000 ease-in-out ${idx === wordIndex ? "opacity-[0.85]" : "opacity-0"}`}
                    style={{
                      WebkitTextStroke: "2px rgba(255, 255, 255, 0.25)",
                      color: "transparent",
                      transform: "translate(-60%, 35%)", // Adjusted transform to align beautifully with "VOTE FOR YOUR" on mobile
                    }}
                  >
                    {word}
                  </div>
